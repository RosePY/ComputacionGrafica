...
const String keys =
    ...
    "{@left          |bL.png | left view of the stereopair                                       }"
    "{@right         |bR.png | right view of the stereopair                                      }"
...
void exportMesh(cv::Mat Depth, cv::Mat Normals, cv::Mat texture) {
  ...
  cv::imwrite("export.png", texture);
  mtlFile.open("export.mtl");
  mtlFile << "newmtl picture" << std::endl;
  mtlFile << "map_Kd export.png" << std::endl;
  objFile.close();
  mtlFile.close();
}
...
Mat texture;
texture= cv::imread("bL.jpg" , CV_LOAD_IMAGE_COLOR);
